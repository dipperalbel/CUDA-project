...
std::vector<uint8_t> pgm_load(unsigned int &width, unsigned int &height, const std::string & file)
{
    ...
    {
        throw std::runtime_error{ "pgm_load failed for " + file };
    }
...
void pgm_save(std::vector<uint8_t> & buffer, const unsigned int width, const unsigned int height, const std::string & file)
{
    ...
    if (!ostr)
    {
        throw std::runtime_error{ "pgm_save failed for " + file };
    }
}
...
std::vector<int16_t> lut_load_fixed(unsigned int &width, unsigned int &height, const std::string & file)
{
    ...
    if (!istr)
    {
        throw std::runtime_error{ "lut_load_fixed failed for " + file };
    }
    std::vector<char> parsedData{ std::istreambuf_iterator<char>(istr), std::istreambuf_iterator<char>() };
...
void save_different_pixels(std::size_t differentPixelsCount, const std::string & file)
{
    std::ofstream ostr(file);
    ostr << differentPixelsCount << "\n";
    if (!ostr)
    {
        throw std::runtime_error{ "save_different_pixels failed for " + file };
    }
}

thrust::device_vector<uint8_t> pgm_load_gpu(unsigned int &width, unsigned int &height, const std::string & file)
{
    std::vector<uint8_t> picture = pgm_load(width, height, file);
    thrust::device_vector<uint8_t> devPicture(picture.size());
    thrust::copy(std::begin(picture), std::end(picture), std::begin(devPicture));
    return devPicture;
}

void pgm_save_gpu(thrust::device_vector<uint8_t> & buffer, const unsigned int width, const unsigned int height, const std::string & file)
{
    std::vector<uint8_t> hostBuffer( buffer.size() );
    thrust::copy(std::begin(buffer), std::end(buffer), std::begin(hostBuffer));
    pgm_save(hostBuffer, width, height, file);
}

thrust::device_vector<int16_t> lut_load_fixed_gpu(unsigned int &width, unsigned int &height, const std::string & file)
{
    std::vector<int16_t> picture = lut_load_fixed(width, height, file);
    thrust::device_vector<int16_t> devPicture(picture.size());
    thrust::copy(std::begin(picture), std::end(picture), std::begin(devPicture));
    return devPicture;
}
